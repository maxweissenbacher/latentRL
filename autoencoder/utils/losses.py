...
class LossTracker:
    ...
    def init_current_loss_dict(self):
        self.current_loss_dict = {
            f"current_{loss_type}": 0 for loss_type in self.loss_types
        }
    ...
    def update_current_loss(self, loss_type, loss_total, loss_dec, loss_enc, loss_reg):
        ...
        self.losses_dict[f"current_{loss_type}_loss"] += (
            loss_total.numpy(force=True) / divider
        )
        self.losses_dict[f"current_{loss_type}_reg_loss"] += (
            loss_reg.numpy(force=True) / divider
        )
        self.losses_dict[f"current_{loss_type}_dec_loss"] += (
            loss_dec.numpy(force=True) / divider
        )
        self.losses_dict[f"current_{loss_type}_enc_loss"] += (
            loss_enc.numpy(force=True) / divider
        )
    ...
    def print_current_loss(self, epoch, loss_type):
        ...
        print(
            f"total loss {loss_dict[current_loss_key]:.2e}; decoded loss {loss_dict[current_dec_loss_key]:.2e}; encoded loss {loss_dict[current_enc_loss_key]:.2e}"
        )
    ...
    def update_wandb_loss(self):
        for loss_type in self.loss_types:
            wandb.log(
                {
                    loss_type: self.losses_dict[loss_type],
                    "epoch": len(self.losses_dict[loss_type]),
                }
            )
